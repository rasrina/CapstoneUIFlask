...
def index():
    ...
    stats = {
        ...
        'rev_table' : df2.groupby(['Category','App']).agg({'Reviews':'max','Rating':'mean'}).sort_values(by="Reviews", ascending=False).head(10).reset_index().to_html(classes=['table thead-light table-striped table-bordered table-hover table-sm'])
    }
    ...
    paid_app = df2[df2['Type']=='Paid'].groupby(['App']).agg({
    'Price':'mean',
    'Rating':'mean',
    'Installs':'sum'
    ...
    top_rev = paid_app.sort_values(by="Revenue",ascending=False).head(10).sort_values(by="Revenue",ascending=True)
    
    X = top_rev.reset_index().App
    Y = top_rev.reset_index().Revenue/1000000
    ...
    plt.savefig('top_rev.png',bbox_inches="tight") 
